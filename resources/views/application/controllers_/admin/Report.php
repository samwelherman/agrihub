...
class Report extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('proposal_model');f
    ...
    public function sales_report($filterBy = null)
    {
        ...
        public function purcharse_report($filterBy = null)
    {
        $data['title'] = lang('stock') . ' ' . lang('report');
        if (!empty($filterBy)) {
            $data['filterBy'] = $filterBy;
            if ($filterBy == 'purcharse' || $filterBy == 'purcharse_by_supplier') {
                $start_date = null;
                $end_date = null;
                if ($filterBy == 'purcharse_by_supplier') {
                    $client = get_result('tbl_suppliers');
                    $status = !empty($supplier) ? $suppler[0]->supplier_id : '0';
                } else {
                    $status = 'all';
                }
