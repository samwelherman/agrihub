                                    ...
                                    <table class="table table-striped" id="table-1">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeA even" role="row">
                                                ...
                                                <td><li><a  href="{{ route('invoicepdf.show', $row->id)}}" ><b>{{ $loop->iteration }}.</b>{{$row->pacel_name}}</a></li></td>
                                        ...
                                        <td>
 <a  class="btn btn-xs btn-outline-info text-uppercase px-2 rounded" title="Edit" onclick="return confirm('Are you sure?')" href="{{ route('pacel.edit', $row->id)}}"><i class="fa fa-edit"></i></a>
{!! Form::open(['route' => ['managepacel.destroy',$row->id], 'method' => 'delete']) !!}
...
{{ Form::close() }}
        ...
        <ul class="dropdown-menu animated zoomIn">
         <li class="nav-item"><a  class="nav-link" title="Edit" data-toggle="modal" class="discount"  href="" onclick="model({{ $row->id }},'discount')" value="{{ $row->id}}" data-target="#appFormModal" >Discount Quotation</a></li>
          <li class="nav-item"><a  class="nav-link" title="Convert to Invoice" onclick="return confirm('Are you sure? you want to convert Quotation To Invoice')"  href="{{ url('convertInvoice', $row->id)}}">Convert to Invoice</a></li>
          <li class="nav-item"><a  class="nav-link" title="Cancel" onclick="return confirm('Are you sure?')"   href="{{ route('pacel.show', $row->id)}}">Cancel Quotation</a></li>                                       
                          </ul></div>
                          ...
                          <select class="form-control" name="owner_id" required id="client">
                                                          @if(!empty($supplier))
                                                          @foreach($supplier as $row)
                                                          @foreach($row->roles as $row2)
                                                          @if($row2->id == 4)
                                                           <option @if(isset($data)) {{  $data->owner_id == $row->id  ? 'selected' : ''}} @endif value="{{ $row->id}}">{{$row->fname}} {{$row->lname}}</option>
                                                          @endif
                                                          @endforeach
                                                          @endforeach
                                                          @endif
                                                  ...
                                                  <button class="btn btn-primary" type="button" data-toggle="modal" onclick="model()" value="" data-target="#appFormModal"><i class="fa fa-plus-circle"></i></button>
                                                  </div>
                                                 </div>
                                                    </div>
                                                </div>
...
<input type="hidden" name="saved_items_id[]" class="form-control item_saved{{$i->order_no}}"   value="{{ isset($i) ? $i->saved_items_id : ''}}" required   />
...
<td><button type="button" name="remove" class="btn btn-danger btn-xs rem" value= "{{ isset($i) ? $i->items_id : ''}}"><i class="fas fa-trash"></i></button></td>
</tr>
...
<script>
 ...
 html += '<input type="hidden" name="saved_items_id[]" class="form-control item_saved'+count+'"  required   />';
